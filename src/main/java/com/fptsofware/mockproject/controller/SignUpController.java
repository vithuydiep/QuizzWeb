...
public class SignUpController {
	...
	public String registerUserAccount(@ModelAttribute("user") UserDTO userDto, 
		...
		try {
			...
			userService.signUpToken(token, email, session);
			
			System.out.println("session token: "+ session.getAttribute("tokenSignUp").toString());
			
			userDto.setEmail(userDto.getUsername());
			userDto.setImage("./assets/image/91448b4b89b3bec0070da6bfd814a3c3.jpg");
			session.setAttribute("sessionSignUpUser", userDto);
			String resetPasswordLink = Utitily.getSiteURL(request) + "/registration/success?token=" + token;
	...
	@GetMapping("/success")
	public String receiEmail(HttpSession session, @Param(value = "token") String token, Model model, @ModelAttribute("user") UserDTO userDto) throws ParseException {
		session.getAttribute("tokenSignUp");
		
		
		if(token.equals(session.getAttribute("tokenSignUp"))) {
			userDto = (UserDTO) session.getAttribute("sessionSignUpUser");
			userService.save(userDto);
			model.addAttribute("message", "Your account already signup!");
		}
